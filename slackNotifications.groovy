...
def afterQG(String QAResult) {
  if(QAResult == "OK"){
    slackSend color: "good", message: "Job: ${env.JOB_NAME} Build: # ${env.BUILD_NUMBER} passed SonarQube Quality Gates!"   
  }
  else if(QAResult == "ERROR") { 
    slackSend color: "danger", message: "Job: ${env.JOB_NAME} Build: # ${env.BUILD_NUMBER} didn't pass SonarQube Quality Gates!"
  }
}

def isRunning(String state){
	if(state == "Running"){
		slackSend color: "good", message: "Job: ${env.JOB_NAME} Build: # ${env.BUILD_NUMBER}. Your app is up and running!"
	}else{
